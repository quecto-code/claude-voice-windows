...
claude-voice は **VOICEVOX 音声合成エンジン**（**Windows 版**、ローカル HTTP。[ADR-0005](../../../adr/0005-windows-native-audio-path.md)）と **vosk 日本語モデル**（ストリーミング確定ワード検出用）を実機で要求する。これらが配置されていないと Epic 02（speak）・Epic 03（listen）の受入確認が成立しない。
...
- [ ] VOICEVOX エンジンが起動でき、PowerShell `Invoke-RestMethod http://127.0.0.1:50021/version` が版番号（HTTP 200）を返す
...
- VOICEVOX の起動をタスクスケジューラ等で自動化すること
- 複数モデルの管理（本プロジェクトは vosk-model-ja-0.22 のみ）

## 補足

- VOICEVOX Windows 版の配布元 / vosk モデルの DL URL は後続 Story 03 で README に明記する（[ADR-0005](../../../adr/0005-windows-native-audio-path.md)）
