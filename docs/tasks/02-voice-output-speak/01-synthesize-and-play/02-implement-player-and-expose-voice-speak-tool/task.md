...
- [x] `src/claude_voice/speak/player.py` に `play(wav: bytes) -> bool` が実装され、`sox -t wav - -t waveaudio default` 相当を起動して stdin に WAV bytes を流し、再生完了までブロックする → mock で subprocess.run の引数（コマンド・input）を確認
...
実装ポイント:
- `play` は `subprocess.run(["sox","-t","wav","-","-t","waveaudio","default"], input=wav, check=False, capture_output=True)`。`check=False` + 自前で returncode 判定
...
- 再生コマンド例: `sox -t wav - -t waveaudio default`（stdin から WAV を流す）
- 関連設計:
