...
Claude が「ユーザに伝えるべき」と判断した内容を日本語音声で読み上げる機能。listen に先行して開発する（speak は listen に依存しないため、最短で「Claude をしゃべらせる」を達成できる）。TTS は VOICEVOX（**Windows 版エンジン**、ローカル HTTP）、再生は `sox` の `waveaudio` 経路（[ADR-0005](../../adr/0005-windows-native-audio-path.md)）。

実装は `src/claude_voice/speak/` 配下に閉じる（`design.md#構成` の独立 2 パイプライン方針）。

## ゴール・成功基準

- `voice_speak(text)` ツールに渡したテキストが VOICEVOX で合成され、Windows の `waveaudio` 経由でスピーカーから再生される。
...
- `sox` による waveaudio 再生（`speak/player.py`）
...
- 音声経路は Windows waveaudio + `sox`（[design.md#全体像](../../design/design.md#全体像) で検証済み・[ADR-0005](../../adr/0005-windows-native-audio-path.md)）
