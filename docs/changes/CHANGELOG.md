...
| CH-003 | 2026-06-05 | tasks→**ADR へ遡上** | modify | 実行環境 / 音声経路 / ADR-0005 / design / requirements / 環境・実装タスク | 実行環境を WSL2（WSLg PulseAudio）から **Windows ネイティブ**へ変更。sox の音声デバイスを `pulseaudio`→`waveaudio`、確定ワード停止を SIGTERM→`proc.terminate()`（Windows に POSIX SIGTERM がないため）、再生の `--buffer`/`rate -v` は WSLg 由来歪み対策から underrun 保険へ位置づけ変更（Audio Quality NFR は存続）。VOICEVOX は Windows 版（HTTP API 同一）、CUDA は `.so`→`.dll`、venv パスは `.venv\Scripts\`。ADR-0003/0004 の決定は不変 | **ADR-0005 新設**。design（全体像・recorder/player 責務・設計判断2・対象スタック・実行環境）/ requirements（Dependencies 実行環境）/ 環境構築タスク（VOICEVOX・venv・vosk・README）/ register-server（.mcp.json Windows パス）/ listen・speak タスク（sox コマンド）/ transcriber（CUDA 前提）→ 実装一式（Windows ネイティブ再実装）まで | applied |
