"""sox 子プロセスで録音し、vosk で確定ワードを検出する recorder（Windows / waveaudio）。
...
- 確定ワード（vosk の partial に検出 → proc.terminate()）  → FinalizeReason.word
- 発話開始が onset timeout 以内に観測されない               → FinalizeReason.timeout

Windows には POSIX SIGTERM がなく、確定ワード時の停止は ``proc.terminate()``
（``TerminateProcess``）になる（ADR-0005）。例外は一切上位に投げず、失敗は
``(b"", FinalizeReason.timeout)`` にマップする（Reliability NFR）。
"""
...
def _build_sox_command(silence_sec: float) -> list[str]:
    """録音用 sox コマンドを組み立てる（Windows: waveaudio 既定デバイス）。
            ...
            while True:
                ...
                try:
                    ...
                    if not word_detected.is_set() and finalize_word in _partial_text(recognizer):
                        word_detected.set()
                        proc.terminate()  # Windows: TerminateProcess（ADR-0005）
    ...
    try:
        while True:
            if proc.poll() is not None:
                # sox が終了した。word なら terminate 起因、そうでなければ無音停止。
